...
This was tried (`a4c02e5`) and reverted. It saved ~47 lines and cost more than it
saved:
...
- and the "duplication" is the **on-disk file format**, not logic. Those offsets
  are fixed by the spec. They cannot drift apart in the way shared code exists to
  prevent, so the coupling a helper would create buys nothing.

## Consequences

Treat the two parsers as independent and acceptable as they are. Per-file
cleanups — clearer comments, fewer intermediate variables — are welcome; hoisting
the parsing into a shared helper is not.
