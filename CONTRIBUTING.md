...
There is deliberately no `prepublishOnly` script: `preversion` gates the local
side and the publish job's `needs: test` gates CI, so adding one just runs
lint/test/build a third time per release.
